...
pub fn login()
{
    ...
    //build_first_header(&mut headers);
    build_second_header(&mut headers);
...
fn build_first_header(headers: &mut header::HeaderMap)
{
    ...
    let cookie = "Tenant=; Letkey=; Landlord=; LetAct=; Test=test; Ref=; Password=; \
    Connection=; UserType=; Position=; Company=; User=;";
...
fn build_second_header(headers: &mut header::HeaderMap)
{
    ...
    let accept_language = "en-GB,en-US;q=0.9,en;q=0.8";
    headers.insert(header::ACCEPT_LANGUAGE, header::HeaderValue::from_static(accept_language));
    let cookie = "ASPSESSIONIDCGQTTBRA=PNOOJOKCGGDEBFKNFDCOPDED; Tenant=; Letkey=; Landlord=; Company=Sellectlets; LetAct=; Position=Consultant; UserType=M; UserFullname=James+Butterworth; User=James; Connection=sellectlets; Test=test; Ref=; Password=tester";
    headers.insert(header::COOKIE, header::HeaderValue::from_static(cookie));
